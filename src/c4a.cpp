/* c4a.cpp */
/* Fri 20 Dec 15:06:30 UTC 2024 */

#define SHA1DATE "a4de913\r\n"
...
#define PRIMS_BASE                                                             \
    X(                                                                         \
        EXIT, "exit", 0, if (0 < rsp) { pc = (wc_t)rpop(); } else { return; }) \
    ...
    X(INDEX, "i", 0, push(L0);)                                                \
    X(                                                                         \
        NEXT, "next", 0, if (++L0 < L1) { pc = (wc_t)L2; } else {              \
            lsp = (lsp < 3) ? 0 : lsp - 3;                                     \
        })                                                                     \
    ...
    X(                                                                         \
        TTO, ">t", 0, t = pop(); if (tsp < TSTK_SZ) { tstk[++tsp] = t; };)     \
    ...
    X(                                                                         \
        TDROP, "tdrop", 0, if (0 < tsp) { tsp--; })                            \
    X(                                                                         \
        TOA, ">a", 0, t = pop(); if (asp < TSTK_SZ) { astk[++asp] = t; })      \
    ...
    X(ZQUOTE, "z\"", 1, quote();)                                              \
    X(DOTQT, ".\"", 1, quote();                                                \
      (state == COMPILE) ? comma(FTYPE) : fType((char *)pop());)               \
    X(FIND, "find", 0, {                                                       \
        DE_T *dp = findWord(0);                                                \
        push(dp ? dp->xt : 0);                                                 \
        push((cell)dp);                                                        \
...
#ifndef FILE_NONE
#define PRIMS_FILE                                                             \
    X(FLOPEN, "fopen", 0, t = pop(); n = pop();                                \
      push(fileOpen((char *)n, (char *)t));)                                   \
    X(FLCLOSE, "fclose", 0, t = pop(); fileClose(t);)                          \
    X(FLDEL, "fdelete", 0, t = pop(); fileDelete((char *)t);)                  \
    X(FLREAD, "fread", 0, t = pop(); n = pop();                                \
      TOS = fileRead((char *)TOS, (int)n, t);)                                 \
    X(FLWRITE, "fwrite", 0, t = pop(); n = pop();                              \
      TOS = fileWrite((char *)TOS, (int)n, t);)                                \
    X(FSEEK, "fseek", 0, t = pop(); TOS = fileSeek(t, TOS);)                   \
    X(FPOS, "fpos", 0, TOS = filePos(TOS);)                                    \
    X(FSIZE, "fsize", 0, TOS = fileSize(TOS);)                                 \
    X(                                                                         \
        LOADED, "loaded?", 0, t = pop(); pop(); if (t) { toIn = inPop(); })    \
...
#define PRIMS_SYSTEM                                                           \
    ...
    X(GOJIRAF, "gojiraflash", 0, gojiraReflash();)                             \
...
char *inPop() {
    return (0 < inSp) ? inStk[inSp--] : 0;
}
int lower(const char c) {
    return btwi(c, 'A', 'Z') ? c + 32 : c;
}
...
void commaCell(cell n) {
    store32((cell)&code[here], n);
    here += (CELL_SZ / WC_SZ);
}
int changeState(int x) {
    state = x;
    return x;
}
...
void ok() {
    ...
    zType((state == INTERP) ? SHA1DATE : " ");
}

wc_t getWCat(cell addr) {
    return code[addr];
}
void setWCat(cell addr, wc_t val) {
    code[addr] = val;
}

int strEqI(const char *s, const char *d) {
    while (lower(*s) == lower(*d)) {
        if (*s == 0) {
            return 1;
        }
        s++;
...
int getWord() {
    ...
    while (btwi(*toIn, 1, 32)) {
        ch = *(toIn++);
        if (btwi(ch, COMPILE, COMMENT)) {
            state = ch;
        }
    }
    while (btwi(*toIn, 33, 126)) {
        wd[len++] = *(toIn++);
    }
    wd[len] = 0;
    return len;
...
int findXT(int xt) {
    mySerial.println(
        " DEBUG: Line 247: c4a.cpp: cast of dp->xt to (int) NON_STANDARD");
    int cw = last;
    while (cw < MEM_SZ) {
        DE_T *dp = (DE_T *)&memory[cw];
        if ((int)dp->xt == xt) {
...
void doSee() {
    ...
    if (dp->xt <= BYE) {
        zTypeF("%s is a primitive (#%ld/$%lX).\r\n", wd, dp->xt, dp->xt);
        return;
    }
    cell x = (cell)dp - (cell)memory;
    int i = dp->xt, stop = (lastWord < dp) ? (dp - 1)->xt : here;
    zTypeF("\r\n%04lX: %s (%04lX to %04lX)", (long)x, dp->nm, (long)dp->xt,
           (long)stop - 1);
    while (i < stop) {
        long op = code[i++];
        zTypeF("\r\n%04X: %04X\t", i - 1, op);
        ...
        case STOP:
            ...
            i += (CELL_SZ / WC_SZ);
            BCASE JMP : zTypeF("jmp $%04lX", (long)x);
            i++;
            BCASE JMPZ : zTypeF("jmpz $%04lX (IF)", (long)x);
            i++;
            BCASE NJMPZ : zTypeF("njmpz $%04lX (-IF)", (long)x);
            i++;
            BCASE JMPNZ : zTypeF("jmpnz $%04lX (WHILE)", (long)x);
            i++;
            break;
            BCASE NJMPNZ : zTypeF("njmpnz $%04lX (-WHILE)", (long)x);
            i++;
            break;
        default:
            x = findXT(op);
...
void compileNum(cell num) {
    if (btwi(num, 0, NUM_MASK)) {
        comma((wc_t)(num | NUM_BITS));
    } else {
        comma(LIT);
        comma(num);
    }
...
next:
    ...
    case STOP:
        return;
        NCASE LIT : push(fetch32((cell)&code[pc]));
        pc += CELL_SZ / WC_SZ;
        ...
        PRIMS_BASE PRIMS_FILE PRIMS_SYSTEM goto next;
    default:
        if ((wc & NUM_BITS) == NUM_BITS) {
            push(wc & NUM_MASK);
            goto next;
        }
        if (code[pc] != EXIT) {
...
int isNum(const char *w, int b) {
    ...
    while (c) {
        n = (n * b);
        if (btwi(c, '0', '9') && btwi(c, '0', '0' + b - 1)) {
            n += (c - '0');
        } else if (btwi(c, 'a', 'a' + b - 11)) {
            n += (c - 'a' + 10);
        } else
            return 0;
        c = lower(*(w++));
...
int isStateChange(const char *wd) {
    ...
    if (strEq(wd, ":")) {
        return changeState(DEFINE);
    }
    if (strEq(wd, "[")) {
        return changeState(INTERP);
    }
    if (strEq(wd, "]")) {
        return changeState(COMPILE);
    }
    if (strEq(wd, "(")) {
        prevState = state;
